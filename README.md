...
terminal and resumes with `agency copilot -- --yolo --resume <session-id>` plus
the per-workspace Symphony `--additional-mcp-config`, so Azure DevOps MCP is
forced for the resumed session even outside a multiplexer.
